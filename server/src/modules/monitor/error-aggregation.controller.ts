 ...
 */
@ApiTags('错误聚合')
@Controller('api/error-aggregations')
export class ErrorAggregationController {
  constructor(private readonly errorAggregationService: ErrorAggregationService) {}
  ...
  async getErrorAggregations(@Query() queryDto: QueryErrorAggregationDto) {
    try {
      const result = await this.errorAggregationService.findErrorAggregations(queryDto);
      return {
        success: true,
        message: '查询成功',
        data: result,
  ...
  async getErrorAggregation(@Param('id') id: string) {
    try {
      const result = await this.errorAggregationService.findErrorAggregationById(parseInt(id));
      if (!result) {
        return {
          success: false,
          message: '错误聚合不存在',
  ...
  async getErrorAggregationStats(
    ...
    try {
      const result = await this.errorAggregationService.getErrorAggregationStats({
        projectId,
