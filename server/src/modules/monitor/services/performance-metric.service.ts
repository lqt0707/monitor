...
import {
  ...
  BatchCreatePerformanceMetricDto,
} from "../dto/create-performance-metric.dto";
...
export class PerformanceMetricService {
   ...
   */
  async create(
    createDto: CreatePerformanceMetricDto
    ...
    try {
      ...
      const metricData = {
        ...
        deviceInfo: createDto.deviceInfo
          ? JSON.stringify(createDto.deviceInfo)
          : null,
        networkInfo: createDto.networkInfo
          ? JSON.stringify(createDto.networkInfo)
          : null,
        customMetrics: createDto.customMetrics
          ? JSON.stringify(createDto.customMetrics)
          : null,
        extraData: createDto.extraData
          ? JSON.stringify(createDto.extraData)
          : null,
      };
   ...
   */
  async createBatch(
    batchDto: BatchCreatePerformanceMetricDto
  ): Promise<PerformanceMetric[]> {
    try {
      const metrics = batchDto.metrics.map((dto) => {
        const metricData = {
          ...
          customMetrics: dto.customMetrics
            ? JSON.stringify(dto.customMetrics)
            : null,
  ...
  async findAll(queryDto: QueryPerformanceMetricDto) {
    ...
    if (platformCode) {
      queryBuilder.andWhere("metric.platformCode = :platformCode", {
        platformCode,
      });
    }
    ...
    if (pageUrl) {
      queryBuilder.andWhere("metric.pageUrl LIKE :pageUrl", {
        pageUrl: `%${pageUrl}%`,
      });
    }
  ...
  async getStatistics(projectId?: string, startDate?: Date, endDate?: Date) {
    try {
      ...
      const typeStats = await this.performanceMetricRepository
        ...
        .where(projectId ? "metric.projectId = :projectId" : "1=1", {
          projectId,
        })
        .andWhere(
          startDate && endDate
  ...
  async cleanupExpiredData(days: number = 30) {
    try {
      ...
      const result = await this.performanceMetricRepository
        .createQueryBuilder()
        .delete()
        .where("metricTimestamp < :cutoffDate", {
