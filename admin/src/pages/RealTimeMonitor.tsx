...
import { GaugeChart } from '../components/AdvancedCharts';
...
const RealTimeMonitor: React.FC = () => {
  ...
  const [errorTrendData, setErrorTrendData] = useState<Array<{ time: string; count: number }>>([
  ...
  const updateRealTimeData = () => {
    ...
    setSystemMetrics(prev => ({
      ...
      requestsPerSecond: Math.max(0, prev.requestsPerSecond + Math.floor((Math.random() - 0.5) * 50))
    }));
    ...
    // 随机添加新错误
    if (Math.random() < 0.3) {
      const errorTypes: RealTimeError['type'][] = ['javascript', 'api', 'network', 'database'];
      const errorLevels: RealTimeError['level'][] = ['error', 'warning', 'info'];
  ...
  return (
    <div style={{ padding: 24 }}>
      <div style={{ marginBottom: 24, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <Title level={2} style={{ margin: 0 }}>实时监控</Title>
        <Space>
          <Text>自动刷新</Text>
          <Switch 
            checked={autoRefresh} 
        ...
        <Col xs={24} sm={12} md={6}>
          <Card>
            <Statistic
              ...
              valueStyle={{ color: systemMetrics.errorRate > 5 ? '#ff4d4f' : '#52c41a' }}
            />
          </Card>
        </Col>
        <Col xs={24} sm={12} md={6}>
          <Card>
            <Statistic
              ...
              valueStyle={{ color: systemMetrics.responseTime > 300 ? '#ff4d4f' : '#52c41a' }}
            />
          </Card>
        </Col>
      </Row>
      ...
      <Row gutter={[16, 16]} style={{ marginBottom: 24 }}>
        <Col xs={24} lg={16}>
          <Card title="错误趋势监控" extra={<Badge status={isMonitoring ? 'processing' : 'default'} text={isMonitoring ? '监控中' : '已暂停'} />}>
            <ReactECharts option={getErrorTrendOption()} style={{ height: '300px' }} />
          </Card>
        </Col>
        <Col xs={24} lg={8}>
          <Card title="系统资源">
            <Space direction="vertical" style={{ width: '100%' }} size="large">
              <div>
                ...
                <Progress 
                  ...
                  status={systemMetrics.cpu > 80 ? 'exception' : systemMetrics.cpu > 60 ? 'normal' : 'success'}
                  showInfo={false}
                />
              </div>
              ...
              <div>
                ...
                <Progress 
                  ...
                  status={systemMetrics.memory > 80 ? 'exception' : systemMetrics.memory > 60 ? 'normal' : 'success'}
                  showInfo={false}
                />
              </div>
              ...
              <div>
                ...
                <Progress 
                  ...
                  status={systemMetrics.disk > 80 ? 'exception' : systemMetrics.disk > 60 ? 'normal' : 'success'}
                  showInfo={false}
                />
              </div>
              ...
              <div>
                ...
                <Progress 
                  ...
                  status={systemMetrics.network > 80 ? 'exception' : systemMetrics.network > 60 ? 'normal' : 'success'}
                  showInfo={false}
                />
              </div>
            </Space>
                  ...
                  <List.Item.Meta
                    ...
                    title={
                      <Space>
                        ...
                        <Badge count={error.count} style={{ backgroundColor: '#ff4d4f' }} />
                      </Space>
                    }
                    description={
                      <Space>
