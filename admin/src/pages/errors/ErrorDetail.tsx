...
const ErrorDetail: React.FC = () => {
      ...
      try {
        ...
        if (errorId) {
          ...
          if (response.success) {
            ...
            throw new Error(response.message || '获取错误详情失败');
          }
        }
      } catch (err) {
  ...
  const errorLevel = errorLevelMap[error.errorLevel as keyof typeof errorLevelMap] || 
  ...
  return (
    <div className="error-detail-container">
      ...
      <Card className="error-summary-card">
        <Descriptions title="错误概要" bordered column={{ xxl: 4, xl: 3, lg: 3, md: 2, sm: 1, xs: 1 }}>
          <Descriptions.Item label="错误消息">{error.errorMessage}</Descriptions.Item>
          <Descriptions.Item label="错误类型">{error.type}</Descriptions.Item>
          <Descriptions.Item label="项目ID">{error.projectId}</Descriptions.Item>
          <Descriptions.Item label="版本">{error.projectVersion || '未知'}</Descriptions.Item>
          <Descriptions.Item label="发生时间">{formatDateTime(error.createdAt)}</Descriptions.Item>
          <Descriptions.Item label="源文件">{error.sourceFile || '未知'}</Descriptions.Item>
          <Descriptions.Item label="行号">{error.sourceLine || '未知'}</Descriptions.Item>
          <Descriptions.Item label="列号">{error.sourceColumn || '未知'}</Descriptions.Item>
        </Descriptions>
      </Card>

      <Tabs activeKey={activeTab} onChange={setActiveTab} className="error-detail-tabs">
        <TabPane 
          tab={<span><HistoryOutlined />错误堆栈</span>} 
          key="1"
        >
            ...
            <div className="source-code-container">
              <Card title="源代码位置" className="source-location-card">
                <Descriptions bordered column={{ xxl: 3, xl: 3, lg: 2, md: 2, sm: 1, xs: 1 }}>
                  <Descriptions.Item label="原始文件">
                    {sourceCode.resolvedFrames?.[0]?.originalSource || error.sourceFile || '未知'}
                  </Descriptions.Item>
                  <Descriptions.Item label="原始行号">
                    {sourceCode.resolvedFrames?.[0]?.originalLine || error.sourceLine || '未知'}
                  </Descriptions.Item>
                  <Descriptions.Item label="原始列号">
                    {sourceCode.resolvedFrames?.[0]?.originalColumn || error.sourceColumn || '未知'}
                  </Descriptions.Item>
                </Descriptions>
              </Card>
              ...
              {sourceCode.sourceCode ? (
                <SourceCodeViewer
                  ...
                  fileName={sourceCode.resolvedFrames?.[0]?.originalSource || error.sourceFile}
                  errorLine={sourceCode.resolvedFrames?.[0]?.originalLine || error.sourceLine}
                  darkMode={true}
                ...
                <Alert
                  message="无法加载源代码"
                  description="未找到对应的源代码文件或源码映射。请确保已上传正确的源代码和 sourcemap 文件。"
                  type="warning"
                  showIcon
                />
              )}
        ...
        <TabPane 
          tab={<span><UserOutlined />用户信息</span>} 
          key="3"
        >
          <Card title="用户与环境信息">
            <Descriptions bordered column={{ xxl: 3, xl: 3, lg: 2, md: 2, sm: 1, xs: 1 }}>
              <Descriptions.Item label="用户ID">{error.userId || '未知'}</Descriptions.Item>
              <Descriptions.Item label="页面URL">{error.pageUrl || '未知'}</Descriptions.Item>
              <Descriptions.Item label="User Agent">{error.userAgent || '未知'}</Descriptions.Item>
            </Descriptions>
