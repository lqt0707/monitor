...
const Dashboard: React.FC = () => {
  ...
  const fetchInitialData = async () => {
    try {
      ...
      await Promise.all([
        dispatch(fetchErrorStats(undefined)),
        dispatch(fetchErrorTrends({ projectId: undefined })),
  ...
  const getProjectErrorsOption = () => {
    ...
    // 使用真实的项目错误统计数据
    const projectCounts = projects.slice(0, 5).map((project: any) => {
      // 从错误聚合中计算每个项目的错误数量
      const projectErrors = errorAggregations.filter(
        (error: any) => error.projectId === project.id
      );
      return projectErrors.reduce(
        (sum: number, error: any) => sum + (error.count || 0),
        0
      );
    });
  ...
  // 计算未解决错误数
  const unresolvedErrors = errorAggregations.filter(
    (e: any) => !e.isResolved
  ).length;
  const resolvedErrors = errorAggregations.filter(
    (e: any) => e.isResolved
  ).length;
  ...
  // 错误状态 - 使用本地错误状态
  if (loadingError || errorError || projectsError) {
    return (
      <Alert
        message="加载失败"
        description={loadingError || errorError || projectsError}
        type="error"
        showIcon
        style={{ margin: "20px 0" }}
      />
  ...
  return (
    <div className="dashboard-container">
      ...
      {(loadingError || errorError || projectsError) && (
        <Alert
          message="数据加载失败"
          description={loadingError || errorError || projectsError}
          type="error"
                      ...
                      <Statistic
                        ...
                        suffix={
                          <div className="stat-trend">
                            ...
                            >
                              {errorTrends.length > 1
                                ? `${
                                    errorTrends[errorTrends.length - 1].count >
                                    errorTrends[errorTrends.length - 2].count
                                      ? "+"
                                      : ""
                                  }
                                  ${Math.round(
                                    ((errorTrends[errorTrends.length - 1]
                                      .count -
                                      errorTrends[errorTrends.length - 2]
                                        .count) /
                                      Math.max(
                                        errorTrends[errorTrends.length - 2]
                                          .count,
