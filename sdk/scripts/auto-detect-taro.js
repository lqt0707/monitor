...
function detectProjectType() {
  ...
  if (packageJson.dependencies) {
    ...
    // 检测监控SDK
    result.hasMonitorSDK = !!(
      packageJson.dependencies['@monitor/sdk']
    );
  }
  ...
  if (fileExists(configPath)) {
    ...
    if (fileExists(indexConfigPath)) {
      ...
      result.hasSourcemap = configContent.includes('source-map') ||
...
function generateConfigurationSuggestions(projectInfo) {
  ...
  } else if (projectInfo.framework === 'vue' || projectInfo.framework === 'vue3') {
    suggestions.push({
      type: 'framework',
      message: 'Vue框架检测',
...
function generateAutoConfigCommands(suggestions) {
  ...
  suggestions.forEach(suggestion => {
    switch (suggestion.type) {
      ...
      case 'sourcemap':
        commands.push({
          command: 'echo "配置sourcemap: 在config/index.ts中添加 chain.devtool(\"source-map\")"',
      ...
      case 'script':
        commands.push({
          command: 'cp node_modules/@monitor/sdk/scripts/build-with-sourcemap.sh . && chmod +x build-with-sourcemap.sh',
...
function displayDetectionResults(projectInfo, suggestions, commands) {
  ...
  logDetail(`Taro项目: ${projectInfo.isTaro ? colors.green('是') : colors.red('否')}`);
  logDetail(`框架: ${projectInfo.framework || colors.yellow('未知')}`);
  logDetail(`TypeScript: ${projectInfo.typescript ? colors.green('是') : colors.yellow('否')}`);
  logDetail(`监控SDK: ${projectInfo.hasMonitorSDK ? colors.green('已安装') : colors.yellow('未安装')}`);
  logDetail(`构建配置: ${projectInfo.buildConfig.hasConfig ? colors.green('已配置') : colors.red('未配置')}`);
  logDetail(`Sourcemap: ${projectInfo.hasSourcemap ? colors.green('已启用') : colors.red('未启用')}`);
  logDetail(`构建脚本: ${projectInfo.buildConfig.hasBuildScript ? colors.green('已存在') : colors.red('未配置')}`);
  console.log();
  ...
  if (suggestions.length > 0) {
    ...
    suggestions.forEach((suggestion, index) => {
      ...
      console.log(`${priorityIcon} [${suggestion.type.toUpperCase()}] ${suggestion.message}`);
